...
import { UserPlus, Users, Edit, Trash2, X, Ticket as Cricket, Search } from 'lucide-react';
...
function App() {
  ...
  const filteredPlayers = players.filter(player => {
    ...
    return (
      ...
      player.age.toString().includes(searchLower)
    );
  });
      ...
      <div className="container mx-auto px-4 py-8">
        ...
        >
          <div className={`relative transition-all duration-300 ${isSearchFocused ? '' : ''}`}>
            <input
              type="text"
              placeholder="Search players by name, role, team..."
              value={searchQuery}
              onChange={(e) => setSearchQuery(e.target.value)}
              onFocus={() => setIsSearchFocused(true)}
              onBlur={() => setIsSearchFocused(false)}
              className="w-full px-6 py-4 bg-white bg-opacity-95 rounded-2xl shadow-lg pl-14 pr-6 text-lg focus:outline-none"
            />
            <Search className={`absolute left-4 top-1/2 transform -translate-y-1/2 w-6 h-6 transition-colors duration-300 ${isSearchFocused ? 'text-purple-600' : 'text-gray-400'}`} />
          </div>
          <AnimatePresence>
            {searchQuery && (
              <motion.div
              ...
              >
                {filteredPlayers.length} results found
              </motion.div>
            )}
          </AnimatePresence>
        </motion.div>
